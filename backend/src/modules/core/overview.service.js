  ...
  async analytics(query = {}) {
    ...
    return cacheService.remember(key, 30, async () => {
      ...
      const [hazards, incidents, nearMisses, trainings, actions, audits, inspections] = await Promise.all([
        ...
        rows(`SELECT MONTH(scheduled_date) month, department_id department, COUNT(*) total, COALESCE(SUM(COALESCE(manhours, participant_count * duration_minutes / 60, 0)),0) manhours FROM training_sessions WHERE ${training.where} GROUP BY month, department_id`, training.replacements),
