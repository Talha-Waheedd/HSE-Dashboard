...
const trainingManhours = (record: any) => {
  if (record.manhours !== null && record.manhours !== undefined) return Number(record.manhours) || 0;
  const participants = Number(record.participantCount ?? record.participant_count ?? record.participants);
  const durationMinutes = Number(record.durationMinutes ?? record.duration_minutes);
  return Number.isFinite(participants) && Number.isFinite(durationMinutes) ? participants * durationMinutes / 60 : 0;
};
...
export const Analytics = ({ focus }: AnalyticsProps = {}) => {
  ...
  const renderProductionSummary = () => {
    const production = (items: any[]) => items.filter(item => String(item.department_id || item.departmentId || item.department || '').toLowerCase().includes('prd') || String(item.department_id || item.departmentId || item.department || '').toLowerCase().includes('production'));
    const hazards = production(current.hazards);
    const incidents = production(current.incidents);
    const nearMisses = production(current.nearMisses);
    const trainings = production(current.trainings);
    const capas = production(current.capas);
    const firstAid = incidents.filter(item => isCategory(item, ['first aid']));
    const recordable = incidents.filter(item => isCategory(item, ['lost time', 'lti', 'restricted', 'rwc', 'medical treatment', 'mtc']));
    const fires = incidents.filter(item => isCategory(item, ['fire']));
    const closed = capas.filter(item => ['closed', 'close', 'completed', 'verified'].includes(String(item.status_id || item.status || '').toLowerCase())).length;
    const monthTrend = Array.from({ length: 12 }, (_, month) => ({ name: new Date(Number(filters.year === 'All' ? new Date().getFullYear() : filters.year), month, 1).toLocaleString('default', { month: 'short' }), value: firstAid.filter(item => yearOf(item) === (filters.year === 'All' ? String(new Date().getFullYear()) : filters.year) && Number(String(item.date || item.incidentDate || item.createdAt || '').slice(5, 7)) - 1 === month).length }));
    const managerNames = Array.from(new Set([...incidents, ...hazards].map(item => String(item.area_manager || item.responsible_person || item.reported_by || 'Unassigned'))).values()).slice(0, 8);
    const managerRows = managerNames.map(name => ({ name, fire: fires.filter(item => String(item.area_manager || item.responsible_person || item.reported_by || 'Unassigned') === name).length, firstAid: firstAid.filter(item => String(item.area_manager || item.responsible_person || item.reported_by || 'Unassigned') === name).length, recordable: recordable.filter(item => String(item.area_manager || item.responsible_person || item.reported_by || 'Unassigned') === name).length }));
    const miniCard = (title: string, actual: number, target: number, color: string) => <div className="border border-[#B7B7B7] bg-white p-3 text-center"><h3 className="min-h-[48px] text-[16px] font-bold">{title}</h3><div className="h-[130px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={[{ name: 'Actual', value: actual }, { name: 'Target', value: target }]} margin={{ top: 20, right: 4, left: -24, bottom: 0 }}><XAxis dataKey="name" tick={{ fontSize: 12, fontWeight: 700 }} /><YAxis hide /><Bar dataKey="value" fill={color} label={{ position: 'top', fontWeight: 700 }} /></BarChart></ResponsiveContainer></div></div>;
    return <Panel title={`Production — ${incidentSummary.monthName} ${filters.year === 'All' ? new Date().getFullYear() : filters.year}`}>
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(420px,.9fr)_minmax(600px,1.1fr)]"><div className="space-y-5"><div className="border border-[#1F2937] p-4 text-[15px] leading-8"><p className="font-bold text-[#0878C1]">{filters.year === 'All' ? new Date().getFullYear() : filters.year}</p><p>Total No. of Recordable Injuries = {recordable.length}</p><p>Total No. of First-Aid Injuries = {firstAid.length}</p><p>Fire Incidents = {fires.length}</p></div><Panel title="First-Aid Incidents"><div className="h-[250px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={monthTrend}><CartesianGrid vertical={false} stroke="#E5E7EB" /><XAxis dataKey="name" /><YAxis allowDecimals={false} /><Tooltip /><Bar dataKey="value" fill="#70AD47" label={{ position: 'top', fontWeight: 700 }} /></BarChart></ResponsiveContainer></div></Panel><Panel title="Under Supervision of"><div className="overflow-x-auto"><table className="min-w-[520px] w-full text-[12px]"><thead><tr><th className="pb-2 text-left">Shift Manager</th><th className="pb-2"># of Fire</th><th className="pb-2"># of FA Incidents</th><th className="pb-2"># of Rec. Injuries</th></tr></thead><tbody>{managerRows.map(row => <tr key={row.name} className="border-t border-[#E5E7EB]"><td className="py-2">{row.name}</td><td className="text-center">{row.fire}</td><td className="text-center">{row.firstAid}</td><td className="text-center">{row.recordable}</td></tr>)}</tbody></table></div></Panel></div><div className="space-y-5"><div className="grid grid-cols-1 gap-4 sm:grid-cols-3">{miniCard('Nearmisses Reporting', nearMisses.length, 48, '#ED7D31')}{miniCard('Hazards Reporting', hazards.length, 540, '#5B9BD5')}{miniCard('Safety Trainings', Math.round(trainings.reduce((sum, item) => sum + (Number(item.manhours || item.total_manhours) || 0), 0)), 3000, '#A5A5A5')}</div><Panel title="Corrective Actions"><div className="grid min-h-[260px] grid-cols-3 border border-[#1F2937] text-center"><div className="flex items-center justify-center border-r border-[#1F2937] p-3 font-bold">Total Corrective Actions Assigned</div><div className="flex items-center justify-center border-r border-[#1F2937] p-3 font-bold">Completed</div><div className="flex items-center justify-center p-3 font-bold">Pending</div><div className="border-r border-t border-[#1F2937] p-6 text-3xl font-bold">{capas.length}</div><div className="border-r border-t border-[#1F2937] p-6 text-3xl font-bold">{closed}</div><div className="border-t border-[#1F2937] p-6 text-3xl font-bold">{capas.length - closed}</div></div></Panel><Panel title="Hazards Closing Status"><div className="flex h-[280px] flex-col items-center justify-center"><div className="h-[220px] w-full"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={[{ name: 'Closed', value: hazards.filter(item => ['closed', 'close'].includes(String(item.status_id || item.status || '').toLowerCase())).length }, { name: 'Open', value: hazards.filter(item => !['closed', 'close'].includes(String(item.status_id || item.status || '').toLowerCase())).length }]} dataKey="value" innerRadius="55%" outerRadius="75%" label><Cell fill="#4472C4" /><Cell fill="#70AD47" /><Legend /></Pie></PieChart></ResponsiveContainer></div><p className="text-center text-xl font-bold text-[#D00000]">Closure Rate = {hazards.length ? Math.round((hazards.filter(item => ['closed', 'close'].includes(String(item.status_id || item.status || '').toLowerCase())).length / hazards.length) * 100) : 0}%</p></div></Panel></div></div>
    </Panel>;
  };

  const renderEsdSummary = () => {
    const scoped = (items: any[]) => items.filter(item => { const value = String(item.department_id || item.departmentId || item.department || '').toLowerCase(); return value.includes('esd') || value.includes('uty') || value.includes('utilities'); });
  ...
  const renderSpecialistAuditsSummary = () => {
    ...
    return <Panel title="Assurance — Specialist Audits & Assessments" className="border-[#2F65AD]">
      <div className="space-y-5">
        <div className="flex items-center gap-4 border-b border-dashed border-[#64748B] pb-4"><img src="/logo.svg" alt="LU" className="h-14 w-auto" /><h2 className="text-2xl font-bold text-[#111827] sm:text-3xl">Audits & Inspections</h2><img src="/image.png" alt="CBL" className="ml-auto h-14 w-14 object-contain" /></div>
        {groups.map(([group, color, items]) => { const groupKey = `${group}-${items.map(([label]) => label).join('|')}`; return <div key={groupKey} className="grid grid-cols-1 gap-4 border-b border-dashed border-[#64748B] pb-5 lg:grid-cols-[300px_1fr]"><div className="flex min-h-20 items-center px-4 py-4 text-lg font-bold text-white [clip-path:polygon(0_0,86%_0,100%_50%,86%_100%,0_100%)]" style={{ backgroundColor: color }}>{group}</div><div className="grid grid-cols-2 gap-4 sm:grid-cols-4 lg:grid-cols-5">{items.map(([label, icon]) => <div key={`${groupKey}-${label}`} className="flex flex-col items-center text-center"><div className="flex h-24 w-24 items-center justify-center border border-[#374151] bg-[#F8FAFC] text-5xl shadow-sm transition hover:scale-105">{icon}</div><p className="mt-2 text-xs font-medium leading-4 text-[#111827] sm:text-sm">{label}</p></div>)}</div></div>; })}
      </div>
    </Panel>;
  };
  ...
  const trainingManhoursForDepartment = (department: string) => {
    const values = overview?.summary?.training?.byDepartment || {};
    const target = department.toLowerCase();
    const match = Object.entries(values).find(([name]) => String(name).toLowerCase() === target || String(name).toLowerCase().includes(target));
