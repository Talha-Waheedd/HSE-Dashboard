...
import { Search, Filter, Activity, FileText, CheckCircle2, Clock, ChevronLeft, ChevronRight } from 'lucide-react';

const PAGE_SIZE = 10;
const pageItems = (current: number, total: number) => {
  if (total <= 7) return Array.from({ length: total }, (_, index) => index + 1);
  const items: Array<number | 'ellipsis'> = [1];
  if (current > 4) items.push('ellipsis');
  for (let page = Math.max(2, current - 2); page <= Math.min(total - 1, current + 2); page += 1) items.push(page);
  ...
  return <Layout>
    ...
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
      ...
      <div className="bg-white border border-[#E5E7EB] rounded-xl shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-3 border-t border-[#E5E7EB] text-[12px] text-[#6B7280]"><span>Page {meta.currentPage} of {Math.max(meta.totalPages, 1)} · {meta.totalRecords} reports</span><div className="flex items-center gap-1"><button aria-label="Previous page" disabled={page <= 1} onClick={() => setPage(value => Math.max(1, value - 1))} className="p-2 rounded border disabled:opacity-40"><ChevronLeft className="w-4 h-4" /></button>{pageItems(meta.currentPage, meta.totalPages).map((item, index) => item === 'ellipsis' ? <span key={`ellipsis-${index}`} className="px-2">…</span> : <button key={item} onClick={() => setPage(item)} className={`min-w-8 h-8 rounded border ${item === meta.currentPage ? 'bg-[#CB0017] text-white border-[#CB0017]' : 'hover:border-[#CB0017]'}`}>{item}</button>)}<button aria-label="Next page" disabled={page >= meta.totalPages} onClick={() => setPage(value => Math.min(meta.totalPages, value + 1))} className="p-2 rounded border disabled:opacity-40"><ChevronRight className="w-4 h-4" /></button></div></div>
      </div>
    </div>
  </Layout>;
};
